#!/usr/bin/env python3
"""Linux worker agent for the HF Space coordinator (dataset_verification)."""
...
def find_python_executable(version: str) -> Optional[Path]:
    """Locate the Python interpreter for a given version in the GitHub tool cache (Linux)."""
...
class VenvManager:
    ...
    def _test_venv_pip_install(self, version: str, venv_dir: Path) -> bool:
        """Run a real pip install (small package) to verify venv functionality."""
        python_exe = self._find_venv_python(venv_dir)
        if python_exe is None:
            return False
        test_pkg = "six"
        try:
            ...
            if result.returncode != 0:
                logger.error(f"pip install test failed for {version}: {result.stderr}")
                return False
            import_result = subprocess.run(
                [str(python_exe), "-c", f"import {test_pkg}"],
                capture_output=True,
                text=True,
                timeout=30,
            )
            if import_result.returncode != 0:
                logger.error(f"import test after pip install failed for {version}: {import_result.stderr}")
                return False
            logger.info(f"pip install test OK for {version}")
            return True
        except Exception as e:
            logger.error(f"pip install test exception for {version}: {e}")
            return False
...
class JobExecutor:
    ...
    def _run_with_venv(self, job: dict, venv_dir: Path) -> dict:
        # venv_dir is already absolute, but ensure it
